...
describe("Table Component header", () => {
  ...
  it("should display table header containing keys passed in data object", () => {
    ...
    const tableHeaderCollection =
      queryByTestId("table-header-row")?.children ?? null;

    const tableHeaderArray = Array.from(tableHeaderCollection ?? []);

    expect(
      tableHeaderArray.map((element) => element.textContent)
    ).toStrictEqual(keys);
  });
  ...
  it("should display custom header if `columnLabels` prop is passed", () => {
    ...
    const { queryByTestId } = render(
      <Table data={dataMock} columnLabels={columnLabels} />
    );
    ...
    expect(
      tableHeaderArray.map((element) => element.textContent)
    ).toStrictEqual(columnLabelsValues);
  });
});
